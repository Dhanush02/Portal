...
@Component({
  selector: 'app-ven-debit',
  templateUrl: './ven-debit.component.html',
  styleUrls: ['./ven-debit.component.css'],
})
export class VenDebitComponent implements OnInit {
  ...
  getInquiryList = () => {
    this.loader = true;
    this.authenticationService.ven_debit().subscribe((data: any) => {
      console.log(data);

      this.inquiryArr =
        data['SOAP:Envelope']['SOAP:Body']['ns0:ZFM_VEN_DEB_DK.Response'][
          'RESULT'
        ]['item'];
      console.log(this.inquiryArr);
      if (data.length > 0) {
        this.dataArr = this.inquiryArr.map((item: any) => {
          console.log(item);
          return {
